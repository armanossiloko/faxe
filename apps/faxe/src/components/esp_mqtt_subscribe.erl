...
-define(META_FIELDS, [<<"seq">>, <<"topic">>, <<"flowid">>, <<"nodeid">>, <<"device">>]).
...
data_received(Topic, Payload,
   ...
   %% seq check
   faxe_seq_check_manager:handle(Item0),
   ...
   Item2 = clean_meta(Item1),
   Item = flowdata:set_root(Item2, As),
   {emit, {1, Item}, S}.

clean_meta(Item = #data_point{fields = #{?META_FIELD := #{} = Meta} = Fields}) ->
   Item#data_point{fields = Fields#{?META_FIELD => maps:with(?META_FIELDS, Meta)}};
clean_meta(Item) -> Item.
