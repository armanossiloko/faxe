...
get_tasks_by_group(GroupName) ->
   case mnesia:dirty_index_read(task, GroupName, #task.group) of
      [#task{} | _] = L -> [convert_from(T) || T=#task{} <- L];
      [] -> {error, not_found}
   end.
...
save_template(#template{id = undefined} = Template0) ->
   Template = convert_to(Template0),
   mnesia:dirty_write(Template#template{id = next_id(template)});
save_template(#template{} = T) ->
   mnesia:dirty_write(convert_to(T)).

-spec save_task(#task{}) -> ok|{error, term()}.
save_task(#task{id = undefined} = Task) ->
   NewTask = sort_tags(convert_to(Task)),
   mnesia:dirty_write(NewTask#task{id = next_id(task)});
save_task(#task{} = T) ->
   NewTask = sort_tags(T),
   mnesia:dirty_write(convert_to(NewTask)).

%%convert_to(#task{definition = Def} = T) when is_map(Def) ->
%%   T#task{definition = maps:to_list(Def)};
%%convert_to(#template{definition = Def} = T) when is_map(Def) ->
%%   T#template{definition = maps:to_list(Def)};
convert_to(T)  -> T.

convert_from(#task{definition = Def} = T) when is_list(Def) ->
   T#task{definition = maps:from_list(Def)};
convert_from(#task{definition = Def} = T) when is_list(Def) ->
   T#template{definition = maps:from_list(Def)};
convert_from(T) -> T.
