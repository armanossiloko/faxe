...
is_authorized(Req) ->
   case (catch cowboy_req:parse_header(<<"authorization">>, Req)) of
      ...
      {bearer, Token} ->
         ...
         case (catch file:read_file(KeyFile)) of
            {ok, PublcPem} ->  {ok, #{<<"preferred_username">> := User} = JwtMap} = jwt:decode(Token, PublcPem),
                                 lager:notice("decoded: ~p", [JwtMap]),
                                 {true, User};
            _ ->
               lager:error("could not read JWT public key file at : ~p", [KeyFile]),
               false
         end;
      _ ->
...
do_register(Req, TaskName, Dfs, Tags, State, Type) ->
   case reg_fun(Dfs, TaskName, Type) of
      ...
      {error, Error} ->
         ...
         lager:warning("Error occured when registering faxe-flow"++Add++" '~p': ~p",[TaskName, Error]),
